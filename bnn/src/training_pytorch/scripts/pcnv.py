...
EXPERIMENT = "[VERSION INFO] 17-JUN-2020. pcnv model, 3x3 with padding=1. Validation test, scales lr every 40 epochs. Can use with FINN."
...
save_path='./results/pcnv_cifar10-w{}a{}.pt'.format(args.wb, args.ab)
prev_acc = 0
def init_weights(m):
...
class cnv(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            #input image: 32x32x3
            BinarizeConv2d(args.wb, 3, 8, kernel_size=3, stride=1, padding=1, bias=True),
            nn.BatchNorm2d(8),
            nn.Hardtanh(inplace=True),
            Quantizer(args.ab),
            #input to maxpool: 32x32x8
            nn.MaxPool2d(kernel_size=2, stride=2),

            #input image: 16x16x8
            BinarizeConv2d(args.wb, 8, 16, kernel_size=3, padding=1, bias=True),
            nn.BatchNorm2d(16),
            nn.Hardtanh(inplace=True),
            Quantizer(args.ab),
            #input to maxpool: 16x16x16
            nn.MaxPool2d(kernel_size=2, stride=2),

            #input image: 8x8x16
            BinarizeConv2d(args.wb, 16, 32, kernel_size=3, padding=1, bias=True),
            nn.BatchNorm2d(32),
            nn.Hardtanh(inplace=True),
            Quantizer(args.ab),
            #input to maxpool: 4x4x32
            nn.MaxPool2d(kernel_size=2, stride=2),
            )
        #input FLAT: 4x4x32
        self.classifier = nn.Sequential(
            BinarizeLinear(args.wb, 32*4*4, 10, bias=False),
            nn.BatchNorm1d(10),
            nn.LogSoftmax()
        )
    ...
    def export(self):
        ...
        save_file = './results/pcnv_cifar10-w{}a{}.npz'.format(args.wb, args.ab)
